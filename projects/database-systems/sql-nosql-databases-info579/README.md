...
- **A data-completeness gap, quantified.** About 10.13% of observation rows (30,363 of 299,697) have no encounter ID recorded at all. A LEFT JOIN check surfaced the gap, the rows load as explicit NULLs, and the rate is documented so encounter-level joins do not silently drop a tenth of the observations.
...
**Missing encounter links (10.13% of observations).** A `LEFT JOIN` check against the encounter table showed that about 10.13% of observation rows (30,363 of 299,697) have no `encounter_id` recorded at all; none reference a non-existent encounter. The loader reads blank fields as NULL, and because `observation.encounter_id` is nullable, those rows load cleanly with explicit NULL links. The real risk is silent loss: any encounter-level join drops those rows, so the rate itself became a documented data-quality finding that encounter-level analyses account for. The pure-SQL version of this taught in the course does the same job by staging the CSV with `LOAD DATA INFILE` and then `INSERT ... SELECT` with a `LEFT JOIN`, so unmatched keys land as visible NULLs; I rebuilt it as a Python loader here so the whole pipeline reproduces with one command.

...
├── setup.R                                         # Installs the 8 required R packages + TinyTeX
...
`setup.R` installs the 8 required packages and bootstraps TinyTeX (a minimal LaTeX distribution, ~150MB) if no system TeX is detected. PDF render takes ~30 seconds after setup.
